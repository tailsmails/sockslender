...
fn start_listener(mut app App, li int) {
	...
	bind_tag := if !l.is_global { ' [isolated-chain: ${l.chain_idxs}]' } else { ' [global-lb]' }
...
fn handle_dns_request(mut app App, mut listener net.UdpConn, client_addr net.Addr, req []u8, l Listener) {
	...
	if l.is_global {
		...
		order = pick_best_from_list(mut app, l.chain_idxs)
	}
...
fn check_http_h(n Node) bool {
	...
	if n.user != '' {
		auth := base64.encode_str('${n.user}:${n.pass}')
...
fn connect_retry(mut app App, host string, port int, l Listener) !&net.TcpConn {
	mut order := []int{}
	if l.is_global {
		order = pick_chain_order(mut app)
	} else {
...
fn connect_chain(chain []Node, host string, port int) !&net.TcpConn {
	...
	mut conn := net.dial_tcp(chain[0].addr) or { return error('dial failed: ${err}') }
...
fn handle_socks5(mut app App, mut client net.TcpConn, l Listener) {
	...
	mut upstream := connect_retry(mut app, host, port, l) or {
		mut f := []u8{len: 10}
		f[0] = 0x05
		f[1] = 0x04
		client.write(f) or {}
...
fn handle_http(mut app App, mut client net.TcpConn, l Listener) {
	...
	if method == 'CONNECT' {
		...
		mut upstream := connect_retry(mut app, t_host, t_port, l) or {
...
fn handle_sni(mut app App, mut client net.TcpConn, l Listener) {
	...
	mut upstream := connect_retry(mut app, hostname, 443, l) or { return }
	defer { upstream.close() or {} }
